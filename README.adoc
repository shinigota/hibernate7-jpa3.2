...
Le `Metamodel` existe depuis JPA 2, pour une présentation courte de la fonctionnalité, voir <<note1>>.
...
Un entity graph peut être défini à partir d'une annotation, dans ce cas : pour charger, pour les projets, uniquement les départements des employés
...
Ou au travers de définition d'objet avec l'API JPA
[source, java]
----
...
L'annotation JPA `@NamedEntityGraph` est inchangée, ici, c'est la 2e solution, l'API JPA qui se retrouve simplifiée :

* Les méthodes sont type safe
* L'API peut utiliser le metamodel
* Les `fetch graph` et `load graph` sont dépréciés

L'équivalent avec JPA 3.2 est :
[source, java]
----
EntityGraph<Shop> eg = em.createEntityGraph(Project.class);
eg.addSubgraph(Project_.EMPLOYEES)
...
.Résumé
****
Cette nouveauté est très importante. +
Les EntityGraph sont très efficaces pour traiter le problème N+1 et sont désormais plus accessibles.
****
...
La clause `SELECT` est facultative dans le cas où ou veut retourner l'entité sélectionnée. +
